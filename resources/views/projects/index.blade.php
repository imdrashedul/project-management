<x-app-layout>
    ...
    <x-slot name="headerButton">
        <x-link-button href="{{ route('projects.create') }}" class="!py-0 !px-2 focus:ring-purple-600">
            {{ __('add') }}
    ...
    <div class="py-12">
        ...
        @if (!$projects->isEmpty())
            @php $offset = ($projects->currentPage() - 1) * $projects->perPage(); @endphp
                            ...
                            <div class="flex gap-4">
                                ...
                                <div class="flex-1 flex flex-col justify-between gap-2">
                                    ...
                                    <div class="flex justify-between gap-4">
                                        ...
                                        @if (!($project->pending_task_count > 0))
                                            <div class="flex flex-col">
                                                <span class="text-sm text-slate-500">Tasks</span>
                                    ...
                                    <div class="flex items-end">
                                        <div class="flex-1">
                                            <p>
                                                {{ Str::limit(html_entity_decode(strip_tags($project->description)), 250, '...') }}
                                            </p>
                                        </div>
                                        <div>
                                            <x-link-button href="{{ route('projects.show', ['project' => $project->ulid]) }}" class="!py-1">View More <i class="fa-solid fa-chevron-right ml-2"></i></x-link-button>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        @else
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 flex flex-col items-center justify-center">
                ...
                <div>
                    <span class="text-xl font-semibold text-slate-400 select-none">No Project Found</span>
                </div>
            </div>
        @endif
    </div>
